...
public interface RoomRepository extends JpaRepository<Room, Long> {
    ...
    @Query("SELECT new map(r.id as room_id, rt.roomName as roomName, rt.description as description, min(r.pricePerDay) as price) FROM Room r join RoomType rt on r.roomTypeId = rt.id where r.isLocked = false GROUP BY r.id, rt.roomName, rt.description order by r.pricePerDay")
