...
public interface ReviewsRepository extends JpaRepository<Reviews, Long> {
    ...
    @Query(value = "SELECT new map(U.name, R.comment) FROM Reviews R JOIN HotelUser U ON R.userId = U.id WHERE R.rating >= 4 ORDER BY R.updInsertTimestamp DESC LIMIT 3")
